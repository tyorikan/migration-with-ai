...
## Step 4: 🎨 A2UI フロントエンド生成（30分）

> **ゴール**: Step 3 の Backend API に対して、ADK Agent + Lit Renderer でリッチな管理画面を自動生成する
...
2. ADK Agent を `a2ui-agent-sdk` + `A2uiSchemaManager` + `BasicCatalog` で構築
3. `get_fast_api_app()` で ADK アプリを生成し、Step 3 の既存 Router を `include_router()` でマージ
4. Lit Renderer をセットアップ
...
Step 1 → 2 → 3 → 4 → 6 が順序通りに実行され、各 Step 完了後にセルフレビュー + `verify-consistency.sh` を実行します。
...
flowchart TD
    ...
    G3 -->|"✅"| S4["Step 4: A2UI 生成"]
    S4 --> CL7["/clear"]
    CL7 --> G4{"/review-gate 4"}
    G4 -->|"✅"| S6["Step 6: ADR 生成"]
    S6 --> DONE["✅ 完了"]
