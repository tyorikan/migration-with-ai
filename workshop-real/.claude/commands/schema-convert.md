...
### 主入力（Step 1 の成果物）
- 統合設計書: `01-reverse-engineering/output/system_overview.md`（ER図・フィールド定義・リレーション）
- Code Wiki: `01-reverse-engineering/output/wiki/objects/`（オブジェクト別詳細）
...
> [!NOTE]
> Wiki にフィールド詳細（型、長さ、Picklist 値等）が十分に記載されている場合は、生の XML を再読みする必要はありません。
> Wiki の情報が不足している場合のみ、補足入力として参照してください。
...
- 出力先: `02-schema-migration/output/generated_ddl.sql`
...
- 出力先: `02-schema-migration/output/data_validation.sql`
